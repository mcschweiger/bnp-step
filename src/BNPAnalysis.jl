...
# Arguments
- `data::Vector{Float32}`: Observed data points.
- `times::Vector{Float32}`: Corresponding time points.

# Returns
A dictionary containing step statistics.
"""
function calculate_step_statistics(data::Vector{Float32}, times::Vector{Float32})
...
"""
    detect_transitions(data::Vector{Float32}, threshold::Float32)

Detects transitions in the data based on a threshold.

# Arguments
- `data::Vector{Float32}`: Observed data points.
- `threshold::Float32`: Threshold for detecting transitions.

# Returns
A vector of indices where transitions occur.
"""
function detect_transitions(data::Vector{Float32}, threshold::Float32)
    transitions = findall(x -> abs(x) > threshold, diff(data))
    return transitions
end

"""
    analyze_step_data(data::Vector{Float32}, times::Vector{Float32}, threshold::Float32)
...
"""
function analyze_step_data(data::Vector{Float32}, times::Vector{Float32}, threshold::Float32)
...
"""
    calculate_transition_rates(transitions::Vector{Int}, times::Vector{Float32})
...
# Returns
The transition rates as a vector of Float32.
"""
function calculate_transition_rates(transitions::Vector{Int}, times::Vector{Float32})
...
"""
    calculate_step_durations(transitions::Vector{Int}, times::Vector{Float32})
...
# Returns
A vector of step durations as Float32.
"""
function calculate_step_durations(transitions::Vector{Int}, times::Vector{Float32})
    if isempty(transitions)
        return Float32[]
    end
...
"""
    calculate_mean_transition_rate(transitions::Vector{Int}, times::Vector{Float32})
...
# Returns
The mean transition rate as a Float32.
"""
function calculate_mean_transition_rate(transitions::Vector{Int}, times::Vector{Float32})
...
"""
    calculate_variance_of_steps(data::Vector{Float32}, transitions::Vector{Int})
...
"""
function calculate_variance_of_steps(data::Vector{Float32}, transitions::Vector{Int})
    if isempty(transitions) || length(transitions) < 2
        return Float32[]
    end
...
"""
    calculate_step_means(data::Vector{Float32}, transitions::Vector{Int})
...
"""
function calculate_step_means(data::Vector{Float32}, transitions::Vector{Int})
    if isempty(transitions) || length(transitions) < 2
        return Float32[]
    end
...
function load_ihmm_mode_means(filename::String, path::Union{String, Nothing}=nothing)
    ...
    for row in CSV.File(full_path)
        append!(heights, parse.(Float32, row))
    end
...
function load_ihmm_mode_mean_trajectory(filename::String, path::Union{String, Nothing}=nothing)
    ...
    sampled_heights = Float32[]
    for row in CSV.File(full_path)
        append!(sampled_heights, parse.(Float32, row))
    end
...
function load_ihmm_samples(filename::String, skip_indices::String, path::Union{String, Nothing}=nothing)
    ...
    for row in CSV.File(full_path)
        ...
        append!(samples, parse.(Float32, row))
        ind_count += 1
    end
...
"""
    parallel_bubble_sort(times::Vector{Float32}, data::Vector{Float32})

Sorts data sets with time points in chronological order.

# Arguments
- `times::Vector{Float32}`: Array of time points.
- `data::Vector{Float32}`: Array of observations.

# Returns
A tuple of sorted time points and sorted observations.
"""
function parallel_bubble_sort(times::Vector{Float32}, data::Vector{Float32})
...
"""
    get_credible_intervals(states::Vector{Float32})

Calculates the credible intervals associated with an array.

# Arguments
- `states::Vector{Float32}`: Array of values for which CI's will be calculated.

# Returns
A tuple containing mean, 95% lower bound, 50% lower bound, median, 50% upper bound, and 95% upper bound.
"""
function get_credible_intervals(states::Vector{Float32})
    mean = mean(states)
...
"""
    remove_burn_in(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                   dt_vec::Matrix{Float32}, f_vec::Vector{Float32}, eta_vec::Vector{Float32}, 
                   post_vec::Vector{Float32}, n::Int)

Removes the first n samples (burn-in).

# Arguments
- `b_vec::Matrix{Float32}`: Array of samples of the loads b_(1:M). Each sample is itself an array of M values.
- `h_vec::Matrix{Float32}`: Array of samples of the step heights h_(1:M). Each sample is itself an array of M values.
- `t_vec::Matrix{Float32}`: Array of samples of the step times t_(1:M). Each sample is itself an array of M values.
- `dt_vec::Matrix{Float32}`: Array of samples of the step durations.
- `f_vec::Vector{Float32}`: Array of samples of the background F_bg.
- `eta_vec::Vector{Float32}`: Array of samples of the noise variance eta.
- `post_vec::Vector{Float32}`: Array of the calculated log posterior for each sample.
- `n::Int`: Number of burn-in samples to discard.

# Returns
A tuple containing cleaned arrays with burn-in removed.
"""
function remove_burn_in(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                        dt_vec::Matrix{Float32}, f_vec::Vector{Float32}, eta_vec::Vector{Float32}, 
                        post_vec::Vector{Float32}, n::Int)
...
"""
    find_map(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
             dt_vec::Matrix{Float32}, f_vec::Vector{Float32}, eta_vec::Vector{Float32}, 
             posteriors::Vector{Float32})
...
- `f_vec::Vector{Float32}`: Array of samples of the background F_bg.
- `eta_vec::Vector{Float32}`: Array of samples of the noise variance eta.
- `posteriors::Vector{Float32}`: Array of the calculated log posterior for each sample.

# Returns
A tuple containing the MAP estimate values for b_m, h_m, t_m, dt_m, F_bg, and eta.
"""
function find_map(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                  dt_vec::Matrix{Float32}, f_vec::Vector{Float32}, eta_vec::Vector{Float32}, 
                  posteriors::Vector{Float32})
...
"""
    find_top_n_samples(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                       f_vec::Vector{Float32}, eta_vec::Vector{Float32}, posteriors::Vector{Float32}, 
...
"""
function find_top_n_samples(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                            f_vec::Vector{Float32}, eta_vec::Vector{Float32}, posteriors::Vector{Float32}, 
                            weak_limit::Int, num_samples::Int=10)
    f_top = zeros(Float32, num_samples)
    eta_top = zeros(Float32, num_samples)
    b_m_top = zeros(Float32, num_samples, weak_limit)
    h_m_top = zeros(Float32, num_samples, weak_limit)
    t_m_top = zeros(Float32, num_samples, weak_limit)
...
"""
    find_top_samples_by_jumps(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                              f_vec::Vector{Float32}, eta_vec::Vector{Float32}, posteriors::Vector{Float32})
...
"""
function find_top_samples_by_jumps(b_vec::Matrix{Float32}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                                   f_vec::Vector{Float32}, eta_vec::Vector{Float32}, posteriors::Vector{Float32})
...
"""
    generate_step_plot_data(b_vec::Vector{Int}, h_vec::Vector{Float32}, t_vec::Vector{Float32}, 
                            dt_vec::Vector{Float32}, f_vec::Float32, weak_limit::Int, 
                            t_n::Vector{Float32}, kernel::Function)
...
- `h_vec::Vector{Float32}`: Array of MAP estimate h_m from a BNP-Step sample.
- `t_vec::Vector{Float32}`: Array of MAP estimate t_m from a BNP-Step sample.
- `dt_vec::Vector{Float32}`: Array of MAP estimate step durations.
- `f_vec::Float32`: MAP estimate F_bg from a BNP-Step sample.
- `weak_limit::Int`: Maximum possible number of steps in the data set.
- `t_n::Vector{Float32}`: Array of time points for the trajectory.
...
"""
function generate_step_plot_data(b_vec::Vector{Int}, h_vec::Vector{Float32}, t_vec::Vector{Float32}, 
                                 dt_vec::Vector{Float32}, f_vec::Float32, weak_limit::Int, 
                                 t_n::Vector{Float32}, kernel::Function)
    ...
    sampled_times = zeros(Float32, jmp_count)
    sampled_heights = zeros(Float32, jmp_count)
...
"""
    generate_gt_step_plot_data(ground_b_m::Vector{Int}, ground_h_m::Vector{Float32}, 
                               ground_t_m::Vector{Float32}, ground_f::Float32, 
                               data_times::Vector{Float32}, weak_limit::Int, kernel::Function)
...
- `ground_h_m::Vector{Float32}`: Ground truth step heights.
- `ground_t_m::Vector{Float32}`: Ground truth step times.
- `ground_f::Float32`: Ground truth value for F_bg.
- `data_times::Vector{Float32}`: Array of time points for the trajectory.
- `weak_limit::Int`: Maximum possible number of steps in the data set.
- `kernel::Function`: Kernel function for step calculation.
...
"""
function generate_gt_step_plot_data(ground_b_m::Vector{Int}, ground_h_m::Vector{Float32}, 
                                    ground_t_m::Vector{Float32}, ground_f::Float32, 
                                    data_times::Vector{Float32}, weak_limit::Int, kernel::Function)
    ...
    ground_times = zeros(Float32, jmp_count_gnd)
    ground_heights = zeros(Float32, jmp_count_gnd)
    ind = 1
    for i in 1:weak_limit
        if ground_b_m[i] == 1
...
"""
    generate_kv_step_plot_data(jump_times::Vector{Float32}, heights::Vector{Float32}, 
                               background::Float32, data_times::Vector{Float32})

Generates step plot data from BIC-based method results.

# Arguments
- `jump_times::Vector{Float32}`: Array of jump times returned by the BIC method.
- `heights::Vector{Float32}`: Array of inter-step means returned by the BIC method. Does not include the final mean.
- `background::Float32`: Value of the final inter-step mean returned by the BIC method.
- `data_times::Vector{Float32}`: Array of time points.

# Returns
A tuple containing the time points and pseudo-observations for the learned trajectory.
"""
function generate_kv_step_plot_data(jump_times::Vector{Float32}, heights::Vector{Float32}, 
                                    background::Float32, data_times::Vector{Float32})
...
"""
    generate_histogram_data(b_vec::Matrix{Int}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                            num_samples::Int, weak_limit::Int, times::Vector{Float32})

Processes raw BNP-Step results into a format that can be histogrammed.

# Arguments
- `b_vec::Matrix{Int}`: Array of b_m from BNP-Step samples.
- `h_vec::Matrix{Float32}`: Array of h_m from BNP-Step samples.
- `t_vec::Matrix{Float32}`: Array of t_m from BNP-Step samples.
...
"""
function generate_histogram_data(b_vec::Matrix{Int}, h_vec::Matrix{Float32}, t_vec::Matrix{Float32}, 
                                 num_samples::Int, weak_limit::Int, times::Vector{Float32})
    histogram_heights = Float32[]
    histogram_lengths = Float32[]
...
"""
    generate_histogram_data_ihmm(samples::Matrix{Float32}, times::Vector{Float32})

Converts iHMM results to step height form for histogramming. Also returns the holding times.

# Arguments
- `samples::Matrix{Float32}`: Array of iHMM samples with the mode number of states.
- `times::Vector{Float32}`: Time points for each observation.

# Returns
A tuple containing arrays of absolute step heights and holding times between the steps.
"""
function generate_histogram_data_ihmm(samples::Matrix{Float32}, times::Vector{Float32})
    histogram_heights = Float32[]
    histogram_times = Float32[]
...
"""
    reconstruct_signal_from_trace(b_m, h_m, t_m, dt_vec, kernel, t_n)

Reconstructs a signal trace from sampled step variables using the given kernel and time vector.
- `b_m`, `h_m`, `t_m` should be vectors over samples.
- `dt_vec` provides the kernel width for each sample (same length as `b_m`).
- `t_n` is the full time trace to evaluate against.
"""
function reconstruct_signal_from_trace(
    b_m::Vector{<:AbstractVector{Bool}},
    ...
    t_n::Vector{Float32};
    mode::Symbol = :last,
    MAP::Union{Int, AbstractVector{Float32},Nothing}
)::Vector{Float32}

@assert !(mode == :MAP && MAP == Nothing) "Inconsistent input signature: If you want to plot the MAP, include a MAP index or posterior trace with MAP = "
if mode == :last
    return reconstruct_signal_from_sample(
        b_m[end],
        h_m[end],
        t_m[end],
        dt_vec[end],
...
elseif mode == :MAP
    MAP = MAP isa Int ? MAP : argmax(MAP)
    return reconstruct_signal_from_sample(
        b_m[MAP],
        h_m[MAP],
        t_m[MAP],
...
"""
    reconstruct_signal_from_sample(b_m, h_m, t_m, dt, kernel, t_n)

Reconstructs a signal trace from sampled step variables using the given kernel and time vector.
- `b_m`, `h_m`, `t_m` should be vectors over samples.
- `dt` provides the kernel width for each sample (same length as `b_m`).
- `t_n` is the full time trace to evaluate against.
"""
function reconstruct_signal_from_sample(
    b_m::AbstractVector{Bool},
    h_m::Vector{Float32},
    t_m::Vector{Float32},
    dt::Float32,
    f::Float32,
    ...
    step_data = zeros(Float32, length(t_n))
    @inbounds for j in eachindex(b_m)
        if b_m[j]
            t_j = t_m[j]
            h_j = h_m[j]
            step_data .+= h_j .* kernel(t_n .- t_j, dt) 
        end
    end
...
export  calculate_step_statistics, detect_transitions, analyze_step_data,
 calculate_transition_rates, calculate_step_durations, calculate_mean_transition_rate, 
 calculate_variance_of_steps, calculate_step_means, load_ihmm_mode_means, 
 load_ihmm_mode_mean_trajectory, kernel, reconstruct_signal_from_trace,reconstruct_signal_from_sample
